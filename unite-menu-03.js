  ...
  desktopNavLink.addEventListener('click', () => {
    ...
    if (isOpen) {
      ...
      gsap.to(whiteDesktopNavLink, {
        color: 'rgba(255,255,255,1)', // Reset filter to none
     ...
     gsap.to(megaMenuAd, {
      ...
      gsap.to(whiteDesktopNavLink, {
        color: 'rgba(218,70,31,1)', // Reset filter to none
        ease: 'power4.inOut', // Easing option
      });
      gsap.to(whiteNavbarLogo, {
...
function closeMegaMenu() {
  ...
  const whiteDesktopNavLinks = document.querySelectorAll('.light-nav-link
');
