...
const loginLink = document.querySelectorAll('.login-link-wrap');
...
linkWraps.forEach(linkWrap => {
  ...
  gsap.set(carrotSpan, { rotation: 0 });
  ...
  desktopNavLink.addEventListener('click', () => {
    ...
    if (isOpen) {
      ...
      gsap.to(carrotSpan, {
        rotation: 0, // Reset rotation to 180 degrees
        duration: 0.4,
     ...
     gsap.to(megaMenuAd, {
      ...
      if (currentOpenLink === linkWrap) {
        ...
        gsap.to(desktopNavLink, {
          color: '#2b2928', // Reset font-weight to 400
          duration: 0.4,
        ...
        gsap.to(loginLink, {
          color: '#ffffff', // Reset font-weight to 400
          duration: 0.4,
          ease: 'power4.inOut', // Easing option for Desktop Nav Link animation
        });
         gsap.to(megaMenuAd, {
      ...
      gsap.to(carrotSpan, {
        rotation: 180, // Rotate the Carrot Span 180 degrees
        duration: 0.4,
        ease: 'power4.inOut', // Easing option for Carrot Span animation
      });

      gsap.to(desktopNavLink, {
        color: '#2a4431', // Set font-weight to 600
        duration: 0,
        ease: 'power4.inOut', // Easing option for Desktop Nav Link animation
      });
  ...
  desktopNavLink.addEventListener('mouseenter', () => {
    if (!linkWrap.classList.contains('open')) {
      gsap.to(desktopNavLink, {
        color: '#2a4431', // Set font-weight to 600 on hover
        duration: 0,
      });
        gsap.to(loginLink, {
          color: '#ffffff', // Reset font-weight to 400
  ...
  desktopNavLink.addEventListener('mouseleave', () => {
    if (!linkWrap.classList.contains('open')) {
      gsap.to(desktopNavLink, {
        color: '#2b2928', // Reset font-weight to 400 on hover out
        duration: 0,
      });
        gsap.to(loginLink, {
          color: '#ffffff', // Reset font-weight to 400
...
function toggleDropdown(menuItem) {
  ...
  if (dropdown) {
    ...
    if (isOpen) {
      ...
      gsap.to(carrotSpan, { rotation: 180, duration: 0.4, ease: 'power4.inOut' }); // Rotate back to default position
    } else {
      closeAllDropdowns();
      showSubmenu(menuItem);
      const submenuHeight = dropdown.offsetHeight;
      gsap.to(menuItem, { height: submenuHeight, duration: 0.8, ease: 'power4.inOut' });
      gsap.to(carrotSpan, { rotation: 0, duration: 0.4, ease: 'power4.inOut' }); // Rotate 180 degrees
